  ...
  return (
    <div className="ytd-admin-modal-backdrop" onMouseDown={event => event.target === event.currentTarget && onClose()}>
      <section className="ytd-admin-book-preview ytd-admin-book-modal" role="dialog" aria-modal="true" aria-labelledby="book-preview-title">
        ...
        <img src={book.cover || BOOK_COVERS[book.title]} alt={`Couverture de ${book.title}`} className="ytd-admin-book-preview-cover" />
        ...
        <p className="ytd-admin-book-preview-description">{book.description || "Aucune description renseignée."}</p>
        <div className="ytd-admin-book-preview-note"><span>Avis éditorial de Yewtod</span><p>{book.note}</p></div>
