...
export default function BookDetail() {
  ...
  return (
    <div className="ytd-book-detail-page" style={{ maxWidth: 1080, margin: "0 auto", padding: "56px 24px 110px" }}>
      ...
      <Breadcrumb items={[{ label: "Home", to: PATHS.home }, { label: "Books", to: PATHS.books }, { label: book.category, to: PATHS.books }, { label: book.title }]} />
      <div className="ytd-book-detail-grid" style={{ display: "grid", gridTemplateColumns: "280px minmax(0, 1fr)", gap: 64, alignItems: "start" }}>
        <div className="ytd-book-detail-cover-wrap">{book.coverImage && <img className="ytd-book-detail-cover" src={book.coverImage} alt={`Couverture de ${book.title}`} style={{ width: "100%", aspectRatio: "2 / 3", objectFit: "cover", border: `1px solid ${T.line}` }} />}<span>Bibliothèque Yewtod</span></div>
        <div className="ytd-book-detail-copy">
          ...
          {book.similarBooks && <section className="ytd-book-detail-section"><h2>Ouvrages similaires</h2><p className="ytd-book-detail-similar">{book.similarBooks}</p></section>}
      ...
      <section className="ytd-book-reviews">
        ...
        <div className="ytd-book-review-list">{reviews.length ? reviews.map(review => <article className="ytd-book-review" key={review.id}><div><strong>{review.author}</strong><small>{new Date(review.date).toLocaleDateString("fr-FR")}</small></div><p>{review.text}</p></article>) : <p className="ytd-book-empty-review">Aucun avis pour le moment. Soyez le premier à partager votre lecture.</p>}</div>
        <form className="ytd-book-review-form" onSubmit={submitReview}><input required value={reviewAuthor} onChange={event => setReviewAuthor(event.target.value)} placeholder="Votre nom" /><textarea required rows={4} value={reviewText} onChange={event => setReviewText(event.target.value)} placeholder="Votre avis sur ce livre" /><Btn type="submit" variant="green"><Send size={15} /> Publier l'avis</Btn></form>
      </section>
      {related.length > 0 && <div className="ytd-book-related"><span className="ytd-dashboard-eyebrow">Continuer la lecture</span><h2>À lire aussi</h2><div>{related.map(item => <button key={item.id} onClick={() => navigate(bookPath(item.slug))}>{item.title} <span>· {item.author}</span></button>)}</div></div>}
    </div>
  );
}
